...
def extract_links(url, output_file=None):
    """Receives a URL, gets the links on that page, creates a file, and returns the created file's name"""
    ...
    if process.returncode == 0:
        try:
            # Attempt to decode the output using 'utf-8'
            html_content = output.decode('utf-8', errors='ignore')
        except UnicodeDecodeError:
            # If 'utf-8' fails, fall back to 'ISO-8859-1'
            html_content = output.decode('ISO-8859-1', errors='ignore')
        
        # Extract links using Python regex
        links = re.findall(r'href="([^"]+)"', html_content)
        
        # Exclude parent directory and symbolic links
        filtered_links = [link for link in links if not link.startswith('#') and not link == '../']
        
        # Determine the output file name
        if not output_file:
            output_file = f"download_links_for_{os.path.basename(os.getcwd())}.txt"
        
        # Write the links to the file with the base link prepended
        with open(output_file, 'w') as f:
            for link in filtered_links:
                if not link.startswith('http'):
                    link = urllib.parse.urljoin(url, link)  # Make relative URLs absolute
                f.write(f"{link}\n")
