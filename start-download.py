...
def parallel_download(file_with_links):
    ...
    command = (
        ...
        f"torify wget -c --no-check-certificate --user-agent=\"Mozilla\" -a $logname {{}}'"
    )
...
def recursive_parallel_download(file_with_links):
    ...
    command = (
        ...
        f"torify wget -c -r -np --no-check-certificate  --user-agent=\"Mozilla\" -a $logname {{}}'"
    )
...
def extract_links(url, output_file=None):
    ...
    wget_command = f'torify wget -qO- --no-check-certificate --user-agent="Mozilla" {url}'
    ...
    if process.returncode == 0:
        ...
        with open(output_file, 'w') as f:
            for link in output.decode().splitlines():
                if link.startswith('/'):
                    f.write(f"{url.rstrip('/')}{link}\n") #remove trailing slash from base url, and append link
                else:
                    f.write(f"{url.rstrip('/')}/{link}\n") #remove trailing slash from base url, and add a slash then the link
